...
if [ -n "${CERT_OPENSSL_BIN:-}" ]; then
	CERT_OPENSSL_BIN=${CERT_OPENSSL_BIN}
elif [ -n "${OPENSSL:-}" ]; then
	CERT_OPENSSL_BIN=${OPENSSL}
elif [ -x "${REPO_ROOT}/tongsuo/apps/openssl" ]; then
	CERT_OPENSSL_BIN="${REPO_ROOT}/tongsuo/apps/openssl"
else
	CERT_OPENSSL_BIN=openssl
fi
CERT_OPENSSL_CONF=${CERT_OPENSSL_CONF:-/dev/null}
