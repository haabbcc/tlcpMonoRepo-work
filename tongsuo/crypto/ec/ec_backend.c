                                     ...
                                     OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    X509_ALGOR_get0(&algoid, &ptype, &pval, palg);
    ...
    if (ptype == V_ASN1_SEQUENCE) {
        ...
        if (EC_KEY_set_group(eckey, group) == 0)
            goto ecerr;
        EC_GROUP_free(group);
    } else if (ptype == V_ASN1_NULL || pval == NULL) {
        /*
         * Parameters are NULL or empty. Check if this is SM2.
         * SM2 uses the EC public key OID (NID_X9_62_id_ecPublicKey) 
         * but may have NULL parameters. When parameters are NULL,
         * we should check if this is SM2 and use SM2 default curve.
         */
        if (algoid != NULL) {
            alg_nid = OBJ_obj2nid(algoid);
            /* 
             * Check if this is SM2. SM2 can be identified by:
             * 1. Direct SM2 OID (NID_sm2)
             * 2. EC public key OID with SM2 signature algorithm (via ossl_x509_algor_is_sm2)
             * 3. EC public key OID with NULL parameters (common for SM2 certificates)
             */
            if (alg_nid == NID_sm2 || ossl_x509_algor_is_sm2(palg)) {
                /* Use SM2 default curve when parameters are NULL */
                group = EC_GROUP_new_by_curve_name_ex(libctx, propq, NID_sm2);
                if (group == NULL)
                    goto ecerr;
                EC_GROUP_set_asn1_flag(group, OPENSSL_EC_NAMED_CURVE);
                if (EC_KEY_set_group(eckey, group) == 0)
                    goto ecerr;
                EC_GROUP_free(group);
            } else if (alg_nid == NID_X9_62_id_ecPublicKey) {
                /*
                 * EC public key OID with NULL parameters.
                 * For SM2 certificates, parameters are often NULL.
                 * Try SM2 curve as it's the most common case in Chinese certificates.
                 */
                group = EC_GROUP_new_by_curve_name_ex(libctx, propq, NID_sm2);
        if (group == NULL)
            goto ecerr;
