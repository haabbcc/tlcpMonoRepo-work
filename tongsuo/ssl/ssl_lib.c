...
SSL *SSL_new(SSL_CTX *ctx)
{
    ...
    s->enable_gm_tls13_dual_cert = ctx->enable_gm_tls13_dual_cert;
    s->enable_sm2_kyber768_tls13 = ctx->enable_sm2_kyber768_tls13;
#endif
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
...
int SSL_CTX_check_private_key(const SSL_CTX *ctx)
{
    ...
    if (EVP_PKEY_is_a(ctx->cert->key->privatekey, "id-ML-DSA-44-SPKI")
        || EVP_PKEY_is_a(ctx->cert->key->privatekey, "ML-DSA-44-SPKI")
        || EVP_PKEY_is_a(ctx->cert->key->privatekey, "id-ML-KEM-512-SPKI")
        || EVP_PKEY_is_a(ctx->cert->key->privatekey, "ML-KEM-512-SPKI"))
        return 1;
                        ...
                        const SSL_METHOD *meth)
{
    ...
    ret->enable_gm_tls13_dual_cert = 0;
    ret->enable_sm2_kyber768_tls13 = 0;
#endif
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
...
void ssl_set_masks(SSL *s)
{
    ...
    if (sm2_enc)
        mask_k |= SSL_kSM2 | SSL_kSM2DHE | SSL_kECCKyber | SSL_kECDHEKyber
                  | SSL_kKyber | SSL_kKyberAigisEnc;

    /*
     * 对于 NTLS 模式，显式开启 ECDHE+Kyber 能力位，避免因证书有效标记未设
     * 导致服务器“无共享套件”。ECDHE_KYBER 需要 ECDHE 能力已开启。
     */
    if (s->enable_ntls)
        mask_k |= SSL_kECDHEKyber | SSL_kKyber | SSL_kKyberAigisEnc;
...
SSL_CTX *SSL_CTX_dup(SSL_CTX *ctx)
{
    ...
    ret->enable_gm_tls13_dual_cert = ctx->enable_gm_tls13_dual_cert;
    ret->enable_sm2_kyber768_tls13 = ctx->enable_sm2_kyber768_tls13;
#endif
...
void SSL_CTX_set_gm_tls13_dual_cert(SSL_CTX *ctx, int enable)
{
    ctx->enable_gm_tls13_dual_cert = (enable != 0);
}

void SSL_set_gm_tls13_dual_cert(SSL *s, int enable)
{
    s->enable_gm_tls13_dual_cert = (enable != 0);
}

void SSL_CTX_enable_sm2_kyber768_tls13(SSL_CTX *ctx)
{
    ctx->enable_sm2_kyber768_tls13 = 1;
}

void SSL_CTX_disable_sm2_kyber768_tls13(SSL_CTX *ctx)
{
    ctx->enable_sm2_kyber768_tls13 = 0;
}
#endif
