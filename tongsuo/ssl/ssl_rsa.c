...
int SSL_use_sign_certificate(SSL *ssl, X509 *x)
{
    ...
    if (EVP_PKEY_is_a(pkey, "SM2"))
        ...
        i = SSL_PKEY_RSA_SIGN;
    else if (EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")
...
int SSL_use_enc_certificate(SSL *ssl, X509 *x)
{
    ...
    if (EVP_PKEY_is_a(pkey, "SM2"))
        ...
        i = SSL_PKEY_RSA_ENC;
    else if (EVP_PKEY_is_a(pkey, "id-ML-KEM-512-SPKI")
             || EVP_PKEY_is_a(pkey, "ML-KEM-512-SPKI"))
        i = SSL_PKEY_SM2_ENC;
    else {
        ERR_raise(ERR_LIB_SSL, SSL_R_WRONG_CERTIFICATE_TYPE);
        return 0;
...
static int ssl_set_cert_idx(CERT *c, X509 *x, int i)
{
    ...
    if (c->pkeys[i].privatekey != NULL) {
        ...
        if (!EVP_PKEY_is_a(c->pkeys[i].privatekey, "id-ML-DSA-44-SPKI")
            && !EVP_PKEY_is_a(c->pkeys[i].privatekey, "ML-DSA-44-SPKI")
            && !EVP_PKEY_is_a(c->pkeys[i].privatekey, "id-ML-KEM-512-SPKI")
            && !EVP_PKEY_is_a(c->pkeys[i].privatekey, "ML-KEM-512-SPKI")
...
static int ssl_set_pkey_idx(CERT *c, EVP_PKEY *pkey, int i)
{
    ...
    if (c->pkeys[i].x509 != NULL) {
        ...
        /*
         * ML-DSA / ML-KEM：EVP_PKEY_eq 与 provider 比对路径可能不完整；
         * 证书与私钥仍须由调用方保证匹配。
         */
        if (!EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")
            && !EVP_PKEY_is_a(pkey, "ML-DSA-44-SPKI")
            && !EVP_PKEY_is_a(pkey, "id-ML-KEM-512-SPKI")
            && !EVP_PKEY_is_a(pkey, "ML-KEM-512-SPKI")
            && !X509_check_private_key(c->pkeys[i].x509, pkey)) {
            X509_free(c->pkeys[i].x509);
            c->pkeys[i].x509 = NULL;
