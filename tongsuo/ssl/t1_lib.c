...
#ifndef OPENSSL_NO_SM2
static int ssl_ctx_append_kyber768_tls13_group(SSL_CTX *ctx)
{
    TLS_GROUP_INFO *ginf;
    TLS_GROUP_INFO *tmp;
    ...
    ginf->tlsname = OPENSSL_strdup("KYBER768");
    ginf->realname = OPENSSL_strdup("KYBER768");
    /*
     * Not a provider key type; ssl_generate_pkey_group rejects this id.
     * Key material uses libcrypto Kyber768 reference implementation.
     */
    ginf->algorithm = OPENSSL_strdup("__KYBER768_TLS13__");
    if (ginf->tlsname == NULL || ginf->realname == NULL || ginf->algorithm == NULL) {
        OPENSSL_free(ginf->tlsname);
        OPENSSL_free(ginf->realname);
        OPENSSL_free(ginf->algorithm);
        memset(ginf, 0, sizeof(*ginf));
    ...
    /*
     * Advertise 256 so tls_group_allowed passes at default library security
     * level 5 (min symmetric bits); ML-KEM-768 is commonly cited ~192-bit.
     */
    ginf->secbits = 256;
    ginf->group_id = OSSL_TLS_GROUP_ID_kyber768;
    ginf->mintls = TLS1_3_VERSION;
    ginf->maxtls = 0;
    ginf->mindtls = -1;
    ginf->maxdtls = -1;
    ginf->is_kem = 1;
...
#ifndef OPENSSL_NO_SM2
    if (!ssl_ctx_append_kyber768_tls13_group(ctx))
        return 0;
#endif
...
/* ML-DSA (Dilithium) for NTLS KYBER_DILITHIUM: digest-less signing */
static const SIGALG_LOOKUP ntls_mldsa_sigalg = {
    "mldsa", 0,
    NID_undef, -1, NID_ML_DSA_44_SPKI, SSL_PKEY_SM2_SIGN,
    NID_undef, NID_undef
};
...
int ssl_setup_sig_algs(SSL_CTX *ctx)
{
    ...
    for (i = 0, lu = sigalg_lookup_tbl;
        ...
        if (!EVP_PKEY_set_type(tmpkey, lu->sig)) {
            /*
             * Legacy EVP_PKEY_set_type failed; try provider-based lookup
             * by algorithm name (covers PQC algorithms like ML-DSA).
             */
            const char *algname = OBJ_nid2sn(lu->sig);
            EVP_PKEY_CTX *pctx2 = NULL;

            if (algname != NULL)
                pctx2 = EVP_PKEY_CTX_new_from_name(ctx->libctx, algname,
                                                     ctx->propq);
            if (pctx2 == NULL) {
                cache[i].enabled = 0;
                continue;
...
int tls1_set_peer_legacy_sigalg(SSL *s, const EVP_PKEY *pkey)
{
    ...
    if (SSL_IS_NTLS(s)) {
        if (EVP_PKEY_is_a(pkey, "SM2"))
            ...
            s->s3.tmp.peer_sigalg = &ntls_rsa_sigalg;
                 ...
                 || EVP_PKEY_is_a(pkey, "ML-DSA-44-SPKI")
                 || EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")) {
            s->s3.tmp.peer_sigalg = &ntls_mldsa_sigalg;
        } else {
            const char *tn = EVP_PKEY_get0_type_name(pkey);

            if (tn != NULL && strstr(tn, "ML-DSA") != NULL)
                s->s3.tmp.peer_sigalg = &ntls_mldsa_sigalg;
            else
                return 0;
        }
    ...
    if (SSL_IS_TLS13(s)) {
        if (EVP_PKEY_is_a(pkey, "ML-DSA")
            || EVP_PKEY_is_a(pkey, "ML-DSA-44-SPKI")
            || EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")) {
            lu = tls1_lookup_sigalg(s, TLSEXT_SIGALG_mldsa44);
            if (lu != NULL) {
...
int tls12_check_peer_sigalg(SSL *s, uint16_t sig, EVP_PKEY *pkey)
{
    ...
    if (pkeyid == -1) {
        if (lu->sigalg == TLSEXT_SIGALG_mldsa44) {
            const char *tn = EVP_PKEY_get0_type_name(pkey);

            if (!(EVP_PKEY_is_a(pkey, "ML-DSA")
                  || EVP_PKEY_is_a(pkey, "ML-DSA-44-SPKI")
                  || EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")
                  || (tn != NULL && strstr(tn, "ML-DSA") != NULL))) {
     ...
     */
    if (SSL_IS_TLS13(s) && (lu->hash == NID_sha1 || lu->hash == NID_sha224))
        goto bad_sig;
    if (lu->sigalg != TLSEXT_SIGALG_mldsa44
 ...
 bad_sig:
    ...
    if (ssl_cert_lookup_by_pkey(pkey, &cidx) == NULL
            && !ssl_cert_lookup_by_nid(EVP_PKEY_get_id(pkey), &cidx)) {
        SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_WRONG_SIGNATURE_TYPE);
        return 0;
    }
    if (lu->sig_idx != (int)cidx) {
        SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_WRONG_SIGNATURE_TYPE);
        return 0;
    }
        ...
        if (SSL_IS_TLS13(s) && s->enable_sm_tls13_strict == 1 && s->server) {
            ...
            if (cipher != NULL &&
                ...
                if (lu->sigalg != TLSEXT_SIGALG_sm2sig_sm3
                    && lu->sigalg != TLSEXT_SIGALG_mldsa44)
                    continue;
            }
        }
#endif
...
#ifndef OPENSSL_NO_SM2
static int tls13_dual_cert_sign_key_is_mldsa(const SSL *s)
{
    EVP_PKEY *pk;
    const char *tn;
    ...
    if (!ssl_has_cert(s, SSL_PKEY_SM2_SIGN))
        return 0;
    pk = s->cert->pkeys[SSL_PKEY_SM2_SIGN].privatekey;
    if (pk == NULL)
        return 0;
    if (EVP_PKEY_is_a(pk, "ML-DSA")
        || EVP_PKEY_is_a(pk, "ML-DSA-44-SPKI")
        || EVP_PKEY_is_a(pk, "id-ML-DSA-44-SPKI"))
        return 1;
    tn = EVP_PKEY_get0_type_name(pk);
    return tn != NULL && strstr(tn, "ML-DSA") != NULL;
}

static int tls13_dual_cert_sign_key_is_sm2(const SSL *s)
{
    EVP_PKEY *pk;
    ...
    if (!ssl_has_cert(s, SSL_PKEY_SM2_SIGN))
        return 0;
    pk = s->cert->pkeys[SSL_PKEY_SM2_SIGN].privatekey;
    return pk != NULL && EVP_PKEY_is_sm2(pk);
}

static int tls13_dual_cert_sigalg_matches_sign_key(const SSL *s,
                                                   const SIGALG_LOOKUP *lu)
{
    if (lu->sigalg == TLSEXT_SIGALG_sm2sig_sm3)
        return tls13_dual_cert_sign_key_is_sm2(s);
    if (lu->sigalg == TLSEXT_SIGALG_mldsa44)
        return tls13_dual_cert_sign_key_is_mldsa(s);
    return 0;
}
#endif
                             ...
                             EVP_PKEY *pkey)
{
     ...
     */
    if (sig->hash == NID_undef && sig->sigalg == TLSEXT_SIGALG_mldsa44) {
        const char *tn = EVP_PKEY_get0_type_name(pkey);

        supported = EVP_PKEY_is_a(pkey, "ML-DSA")
            || EVP_PKEY_is_a(pkey, "ML-DSA-44-SPKI")
            || EVP_PKEY_is_a(pkey, "id-ML-DSA-44-SPKI")
            || (tn != NULL && strstr(tn, "ML-DSA") != NULL);
        if (!supported)
...
static const SIGALG_LOOKUP *find_sig_alg(SSL *s, X509 *x, EVP_PKEY *pkey)
{
    ...
    for (i = 0; i < s->shared_sigalgslen; i++) {
        ...
        if (!tls1_lookup_md(s->ctx, lu, NULL))
            continue;
        if (pkey == NULL) {
            if (!has_usable_cert(s, lu, -1)) {
#ifndef OPENSSL_NO_SM2
                if (!(SSL_IS_TLS13(s) && s->server && s->ctx->enable_gm_tls13_dual_cert
                        && ssl_has_cert(s, SSL_PKEY_SM2_SIGN)
                        && tls13_dual_cert_sigalg_matches_sign_key(s, lu)))
#endif
                    continue;
            }
        } else if (!is_cert_usable(s, lu, x, pkey)) {
            continue;
        }

#ifndef OPENSSL_NO_SM2
        /*
         * RFC 8998 strict mode: GM TLS 1.3 suites require sm2sig_sm3 or
         * mldsa44 (when PQC dual-cert is active).
         */
        if (SSL_IS_TLS13(s) && s->enable_sm_tls13_strict == 1 && s->server) {
            const SSL_CIPHER *cipher = s->s3.tmp.new_cipher;
...
#ifndef OPENSSL_NO_SM2
        if (pkey == NULL && SSL_IS_TLS13(s) && s->server && s->ctx->enable_gm_tls13_dual_cert
                && tls13_dual_cert_sigalg_matches_sign_key(s, lu)
                && ssl_has_cert(s, SSL_PKEY_SM2_SIGN))
            tmppkey = s->cert->pkeys[SSL_PKEY_SM2_SIGN].privatekey;
#endif
...
int tls_choose_sigalg_ntls(SSL *s, int fatalerrs)
{
    ...
    if (kalg & (SSL_kKyber | SSL_kKyberAigisEnc)) {
        lu = &ntls_mldsa_sigalg;
    } else if (kalg & (SSL_kSM2 | SSL_kSM2DHE | SSL_kECCKyber | SSL_kECDHEKyber)) {
        lu = &ntls_sm2_sigalg;
...
int tls_choose_sigalg(SSL *s, int fatalerrs)
{
    ...
    if (dc_lu) {
        ...
        if (SSL_IS_TLS13(s) && s->server && s->ctx->enable_gm_tls13_dual_cert
            ...
            s->s3.tmp.tls13_gm_enc_cert = &s->cert->pkeys[SSL_PKEY_SM2_ENC];
        else
            s->s3.tmp.tls13_gm_enc_cert = NULL;
#endif
        s->s3.tmp.sigalg = lu;
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
    }
