                                     ...
                                     int clearbufs, int stop)
{
    ...
    /* Log cipher suite information after handshake completes */
    if (cleanuphand) {
        const SSL_CIPHER *cipher = s->session != NULL ? s->session->cipher : NULL;
        const char *role = s->server ? "Server" : "Client";

        if (cipher != NULL) {
            fprintf(stderr,
                    "[NTLS %s] Handshake completed - Cipher: %s (0x%04X), Protocol: %s\n",
                    role, SSL_CIPHER_get_name(cipher),
                    (unsigned int)(cipher->id & 0xFFFF),
                    SSL_CIPHER_get_version(cipher));
        } else {
            fprintf(stderr,
                    "[NTLS %s] WARNING: Handshake completed but session->cipher is NULL!\n",
                    role);
        }
    }
...
unsigned char *x509_to_asn1_ntls(const X509 *x, size_t *len)
{
    ...
    p = buf + 3;
    {
        ...
        /* Second DER length can exceed the first (lazy encoding); grow buffer. */
        if ((size_t)n2 + 3 > buflen) {
            OPENSSL_free(buf);
            buflen = (size_t)n2 + 3;
            buf = OPENSSL_malloc(buflen);
            if (buf == NULL)
    ...
    /* l2n3 advances its pointer argument by 3; do not pass |buf| itself. */
    {
        unsigned char *q = buf;
...
int ssl_derive_ntls(SSL *s, EVP_PKEY *privkey, EVP_PKEY *pubkey, int gensecret)
{
    ...
    /* For ECC+Kyber hybrid, the shared secret is already in s->s3.tmp.pms */
    ...
    if (alg_k & SSL_kECCKyber) {
        ...
        if (gensecret) {
            rv = ssl_gensecret(s, s->s3.tmp.pms, s->s3.tmp.pmslen);
        } else {
            /* Already stored in s->s3.tmp.pms */
            rv = 1;
        }
        return rv;
    }
    ...
    /*
     * For ECDHE-based NTLS (SM2DHE or ECDHE_KYBER), derive using provided
     * ephemeral key pair via standard ECDH without SM2-specific params.
     */
    if (alg_k & (SSL_kSM2DHE | SSL_kECDHEKyber)) {
        EVP_PKEY_CTX *ecdh_ctx = NULL;
        size_t ecdh_secret_len = 0;
        unsigned char *ecdh_secret = NULL;

        fprintf(stderr, "[ssl_derive_ntls] ECDHE派生: alg_k=0x%lx, gensecret=%d, priv=%s, pub=%s\n",
                alg_k, gensecret,
                EVP_PKEY_get0_type_name(privkey),
                EVP_PKEY_get0_type_name(pubkey));
        /*
         * ECDHE-Kyber uses plain ECDH for the ECC part. If keys are SM2-typed,
         * force alias to EC for derive path compatibility.
         */
        if ((alg_k & SSL_kECDHEKyber) != 0) {
            if (EVP_PKEY_is_a(privkey, "SM2")
                    && !EVP_PKEY_set_alias_type(privkey, EVP_PKEY_EC)) {
                fprintf(stderr, "[ssl_derive_ntls] 错误: privkey SM2->EC 别名切换失败\n");
                SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
            if (EVP_PKEY_is_a(pubkey, "SM2")
                    && !EVP_PKEY_set_alias_type(pubkey, EVP_PKEY_EC)) {
                fprintf(stderr, "[ssl_derive_ntls] 错误: pubkey SM2->EC 别名切换失败\n");
                SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
        }

        ecdh_ctx = EVP_PKEY_CTX_new_from_pkey(s->ctx->libctx, privkey, s->ctx->propq);
        if (ecdh_ctx == NULL) {
            fprintf(stderr, "[ssl_derive_ntls] 错误: 无法创建EVP_PKEY_CTX\n");
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
            return 0;
        }
        if (EVP_PKEY_derive_init(ecdh_ctx) <= 0) {
            fprintf(stderr, "[ssl_derive_ntls] 错误: EVP_PKEY_derive_init失败\n");
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            EVP_PKEY_CTX_free(ecdh_ctx);
            return 0;
        }
        if (EVP_PKEY_derive_set_peer(ecdh_ctx, pubkey) <= 0) {
            fprintf(stderr, "[ssl_derive_ntls] 错误: EVP_PKEY_derive_set_peer失败\n");
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            EVP_PKEY_CTX_free(ecdh_ctx);
            return 0;
        }
        if (EVP_PKEY_derive(ecdh_ctx, NULL, &ecdh_secret_len) <= 0) {
            fprintf(stderr, "[ssl_derive_ntls] 错误: EVP_PKEY_derive(获取长度)失败, len=%zu\n", ecdh_secret_len);
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            EVP_PKEY_CTX_free(ecdh_ctx);
            return 0;
        }
        fprintf(stderr, "[ssl_derive_ntls] ECDH共享秘密长度: %zu\n", ecdh_secret_len);
        ecdh_secret = OPENSSL_malloc(ecdh_secret_len);
        if (ecdh_secret == NULL) {
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
            EVP_PKEY_CTX_free(ecdh_ctx);
            return 0;
        }
        if (EVP_PKEY_derive(ecdh_ctx, ecdh_secret, &ecdh_secret_len) <= 0) {
            SSLfatal_ntls(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            OPENSSL_clear_free(ecdh_secret, ecdh_secret_len);
            EVP_PKEY_CTX_free(ecdh_ctx);
            return 0;
        }
        EVP_PKEY_CTX_free(ecdh_ctx);
        ...
        if (gensecret) {
            rv = ssl_gensecret(s, ecdh_secret, ecdh_secret_len);
            OPENSSL_clear_free(ecdh_secret, ecdh_secret_len);
        } else {
            s->s3.tmp.pms = ecdh_secret;
            s->s3.tmp.pmslen = ecdh_secret_len;
            rv = 1;
        }
        return rv;
    }
