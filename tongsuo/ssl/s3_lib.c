...
static void sm2kyber_debug_dump(const char *tag, const unsigned char *buf, size_t len)
{
    ...
    if (!sm2kyber_debug_enabled())
        return;
    show = len < 24 ? len : 24;
    fprintf(stderr, "[SM2KYBER-DBG][kdf] %s len=%zu hex=", tag, len);
    for (i = 0; i < show; i++)
...
#  endif    /* OPENSSL_NO_SM3 */
    ...
    {
     1,
     NTLS_TXT_ECC_KYBER_SM4_GCM_SM3,
     NTLS_GB_ECC_KYBER_SM4_GCM_SM3,
     NTLS_CK_ECC_KYBER_SM4_GCM_SM3,
     SSL_kECCKyber,
     SSL_aSM2,
     SSL_SM4GCM,
     SSL_AEAD,
    ...
    {
     1,
     NTLS_TXT_ECDHE_KYBER_SM4_GCM_SM3,
     NTLS_GB_ECDHE_KYBER_SM4_GCM_SM3,
     NTLS_CK_ECDHE_KYBER_SM4_GCM_SM3,
     SSL_kECDHEKyber,
     SSL_aSM2,
     SSL_SM4GCM,
     SSL_AEAD,
    ...
    {
     1,
     NTLS_TXT_KYBER_DILITHIUM_SM4_GCM_SM3,
     NTLS_GB_KYBER_DILITHIUM_SM4_GCM_SM3,
     NTLS_CK_KYBER_DILITHIUM_SM4_GCM_SM3,
     SSL_kKyber,
     SSL_aDilithium,
     SSL_SM4GCM,
     SSL_AEAD,
    ...
    {
     1,
     NTLS_TXT_KYBER_AIGIS_ENC_DILITHIUM_SM4_GCM_SM3,
     NTLS_GB_KYBER_AIGIS_ENC_DILITHIUM_SM4_GCM_SM3,
     NTLS_CK_KYBER_AIGIS_ENC_DILITHIUM_SM4_GCM_SM3,
     SSL_kKyberAigisEnc,
     SSL_aDilithium,
     SSL_SM4GCM,
     SSL_AEAD,
...
void ssl3_free(SSL *s)
{
    ...
    OPENSSL_clear_free(s->s3.kyber768_cli_sk, s->s3.kyber768_cli_sk_len);
    s->s3.kyber768_cli_sk = NULL;
    s->s3.kyber768_cli_sk_len = 0;
    OPENSSL_free(s->s3.peer_kyber768_pk);
    s->s3.peer_kyber768_pk = NULL;
    s->s3.peer_kyber768_pk_len = 0;
#endif
...
#ifndef OPENSSL_NO_SM2
                    && !(s->ctx->enable_gm_tls13_dual_cert
...
EVP_PKEY *ssl_generate_pkey_group(SSL *s, uint16_t id)
{
...
#ifndef OPENSSL_NO_SM2
    if (id == OSSL_TLS_GROUP_ID_kyber768) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
    }
#endif
    ...
    /*
     * ECDHE-Kyber uses plain ECDH for the classical branch. When the selected
     * group is SM2, generating an SM2-typed ephemeral key breaks EVP_PKEY_derive
     * on the server path; force an EC key instead.
     */
    if (id == TLSEXT_curve_SM2
            && (!SSL_is_server(s) || (alg_k & SSL_kECDHEKyber) != 0))
...
#ifndef OPENSSL_NO_SM2
/*
 * PMS for TLS 1.3 HKDF = ECDH shared secret || Kyber768 shared secret (32 bytes).
 * Consumes and clears s->s3.tmp.pms (ECDH leg). |kyber_ss| is overwritten by caller.
 * The ECDH leg is the TLS 1.3 SM2 shared secret from ssl_derive (not the
 * NTLS ecc_kyber_hybrid module, which targets P-256-sized EC points).
 */
int ssl_sm2_kyber768_tls13_combine_and_gensecret(SSL *s,
                                                 const unsigned char *kyber_ss,
                                                 size_t kyber_ss_len)
{
    unsigned char *ecdh = s->s3.tmp.pms;
    size_t ecdh_len = s->s3.tmp.pmslen;
    unsigned char *comb = NULL;
    size_t total;
    int ok = 0;

    if (ecdh == NULL || ecdh_len == 0 || kyber_ss == NULL
            || kyber_ss_len != pqcrystals_kyber768_BYTES) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    total = ecdh_len + kyber_ss_len;
#ifndef OPENSSL_NO_SM2
    if (sm2kyber_debug_enabled()) {
        fprintf(stderr, "[SM2KYBER-DBG][kdf] combine inputs: ecdh_len=%zu kyber_len=%zu total=%zu\n",
                ecdh_len, kyber_ss_len, total);
    }
    sm2kyber_debug_dump("ECDH shared secret", ecdh, ecdh_len);
    sm2kyber_debug_dump("Kyber shared secret", kyber_ss, kyber_ss_len);
#endif
    comb = OPENSSL_malloc(total);
    if (comb == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    memcpy(comb, ecdh, ecdh_len);
    memcpy(comb + ecdh_len, kyber_ss, kyber_ss_len);
#ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("Hybrid IKM (ECDH||Kyber)", comb, total);
#endif
    OPENSSL_clear_free(s->s3.tmp.pms, s->s3.tmp.pmslen);
    s->s3.tmp.pms = NULL;
    s->s3.tmp.pmslen = 0;
    if (ssl_gensecret(s, comb, total))
        ok = 1;
    OPENSSL_clear_free(comb, total);
    return ok;
}
#endif
...
#ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("ssl_derive output shared secret", pms, pmslen);
    if (sm2kyber_debug_enabled()) {
        fprintf(stderr, "[SM2KYBER-DBG][kdf] ssl_derive gensecret=%d\n", gensecret);
    }
#endif
