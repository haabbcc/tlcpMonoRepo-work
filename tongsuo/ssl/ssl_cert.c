...
const SSL_CERT_LOOKUP *ssl_cert_lookup_by_pkey(const EVP_PKEY *pk, size_t *pidx)
{
...
#ifndef OPENSSL_NO_NTLS
    /*
     * KYBER_DILITHIUM 套件使用 SSL_aDilithium（与 SSL_aSM2 同值）。pqmagic 等 provider
     * 的 ML-DSA 公钥可能无法通过上表 OBJ_nid2sn 匹配，但 EVP_PKEY_is_a / 类型名
     * 仍可为 ML-DSA。
     */
    if (EVP_PKEY_is_a(pk, "ML-DSA")
        ...
        if (pidx != NULL)
            *pidx = SSL_PKEY_SM2_SIGN;
        return ssl_cert_lookup_by_idx(SSL_PKEY_SM2_SIGN);
    }
    if (EVP_PKEY_is_a(pk, "ML-KEM-512-SPKI")
        || EVP_PKEY_is_a(pk, "id-ML-KEM-512-SPKI")) {
        if (pidx != NULL)
            *pidx = SSL_PKEY_SM2_ENC;
        return ssl_cert_lookup_by_idx(SSL_PKEY_SM2_ENC);
    }
    {
        const char *tn = EVP_PKEY_get0_type_name(pk);
        ...
        if (tn != NULL && strstr(tn, "ML-KEM") != NULL) {
            if (pidx != NULL)
                *pidx = SSL_PKEY_SM2_ENC;
            return ssl_cert_lookup_by_idx(SSL_PKEY_SM2_ENC);
        }
