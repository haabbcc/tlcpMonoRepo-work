...
# define SSL_kECCKyber           0x00001000U
/* Kyber-only KEX (e.g. KYBER_DILITHIUM_SM4_GCM_SM3): signed_params only, KEM uses enc cert */
# define SSL_kKyber              0x00004000U
# define SSL_kECDHEKyber         0x00002000U
/* ML-KEM（加密证书）+ 临时 Aigis-Enc（provider）双 KEM，签名 ML-DSA（KYBER_AIGIS_ENC_DILITHIUM_SM4_GCM_SM3） */
# define SSL_kKyberAigisEnc      0x00008000U
...
# define SSL_aDilithium          SSL_aSM2
...
typedef enum tlsext_index_en {
    ...
    TLSEXT_IDX_tls1_3_gm_dual_cert,
    TLSEXT_IDX_padding,
    TLSEXT_IDX_psk,
...
# endif
    ...
    int enable_gm_tls13_dual_cert;
    int enable_sm2_kyber768_tls13;
#endif
#ifndef OPENSSL_NO_QUIC
...
# endif
# ifndef OPENSSL_NO_SM2
            /* TLS 1.3 GM dual: encryption certificate (second in chain) */
            CERT_PKEY *tls13_gm_enc_cert;
# endif
# ifndef OPENSSL_NO_NTLS
            CERT_PKEY *sign_cert;
            CERT_PKEY *enc_cert;
            /*
             * KYBER_AIGIS_ENC_DILITHIUM：服务端为临时 Aigis-Enc 密钥对；客户端为服务端下发的公钥
             *（EVP_PKEY_fromdata）。
             */
            EVP_PKEY *aigis_kem_pkey;
# endif
            /*
...
# ifndef OPENSSL_NO_SM2
        /*
         * TLS 1.3 SM2 + Kyber768: second KeyShare uses group OSSL_TLS_GROUP_ID_kyber768;
         * Kyber secret key (client) / peer encapsulation key bytes (server).
         */
        unsigned char *kyber768_cli_sk;
        size_t kyber768_cli_sk_len;
        unsigned char *peer_kyber768_pk;
        size_t peer_kyber768_pk_len;
# endif
...
#ifndef OPENSSL_NO_SM2
        /*
         * ClientHello: server sets if client offered TLSEXT_TYPE_tls1_3_gm_dual_cert.
         * Both sides: 1 if EncryptedExtensions negotiated dual-cert deferred MS.
         */
        uint8_t tls13_gm_dual_cert_request;
        uint8_t tls13_gm_dual_cert;
#endif
    } ext;
...
# ifndef OPENSSL_NO_SM2
    ...
    int enable_gm_tls13_dual_cert;
    int enable_sm2_kyber768_tls13;
    unsigned char tls13_gm_master_secret_ikm[EVP_MAX_MD_SIZE];
    size_t tls13_gm_master_secret_ikm_len;
    /*
     * GM TLS 1.3 dual-cert defers master_secret until Client Finished; we must
     * still derive server application traffic with Transcript-Hash(Server
     * Finished), not the hash after Client Finished. Saved in SW_FINISHED
     * post_work and consumed when switching SERVER_WRITE to application keys.
     */
    uint8_t tls13_gm_dual_cert_sf_hash_saved;
    EVP_PKEY *tls13_peer_gm_enc_pkey;
# endif
...
#define TLSEXT_SIGALG_mldsa44                                   0xFE01
...
# ifndef OPENSSL_NO_SM2
static ossl_inline int ssl_tls13_gm_dual_cert_active(const SSL *s)
{
    return s != NULL && SSL_IS_TLS13(s) && s->ext.tls13_gm_dual_cert != 0;
}
# endif
...
#ifndef OPENSSL_NO_SM2
__owur int ssl_sm2_kyber768_tls13_combine_and_gensecret(SSL *s,
    const unsigned char *kyber_ss, size_t kyber_ss_len);
#endif
...
__owur unsigned long ssl3_output_cert_chain(SSL *s, WPACKET *pkt,
                                            CERT_PKEY *cpk);
# ifndef OPENSSL_NO_SM2
__owur int ssl3_output_cert_chain_gm_dual(SSL *s, WPACKET *pkt, CERT_PKEY *cpk,
                                          X509 *enc);
# endif
