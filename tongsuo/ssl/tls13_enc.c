...
static void sm2kyber_debug_dump(const char *tag, const unsigned char *buf, size_t len)
{
    ...
    if (!sm2kyber_debug_enabled())
        return;
    show = len < 24 ? len : 24;
    fprintf(stderr, "[SM2KYBER-DBG][tls13] %s len=%zu hex=", tag, len);
    for (i = 0; i < show; i++)
                                ...
                                size_t insecretlen)
{
    ...
    int ok = tls13_generate_secret(s, ssl_handshake_md(s), s->early_secret,
                                   insecret, insecretlen,
                                   (unsigned char *)&s->handshake_secret);
#ifndef OPENSSL_NO_SM2
    if (sm2kyber_debug_enabled()) {
        int mdleni = EVP_MD_get_size(ssl_handshake_md(s));
        size_t hlen = mdleni > 0 ? (size_t)mdleni : 0;
        fprintf(stderr, "[SM2KYBER-DBG][tls13] generate_handshake_secret insecret_len=%zu ok=%d\n",
                insecretlen, ok);
        if (insecret != NULL && insecretlen > 0)
            sm2kyber_debug_dump("Handshake secret input IKM", insecret, insecretlen);
        if (ok && hlen > 0)
            sm2kyber_debug_dump("Handshake secret output", s->handshake_secret, hlen);
    }
#endif
    return ok;
}
                                 ...
                                 size_t *secret_size)
{
    ...
    *secret_size = (size_t)mdleni;
#ifndef OPENSSL_NO_SM2
    if (s->tls13_gm_master_secret_ikm_len != 0) {
        if (s->tls13_gm_master_secret_ikm_len != *secret_size) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
        ikm = s->tls13_gm_master_secret_ikm;
        ikmlen = s->tls13_gm_master_secret_ikm_len;
    }
    if (sm2kyber_debug_enabled()) {
        fprintf(stderr, "[SM2KYBER-DBG][tls13] generate_master_secret prevlen=%zu gm_ikm_len=%zu\n",
                prevlen, ikmlen);
        if (ikm != NULL && ikmlen > 0)
            sm2kyber_debug_dump("GM dual-cert IKM input", ikm, ikmlen);
    }
    /* Calls SSLfatal() if required */
    if (!tls13_generate_secret(s, md, prev, ikm, ikmlen, out))
        return 0;
    if (sm2kyber_debug_enabled())
        sm2kyber_debug_dump("Master secret output", out, *secret_size);
    OPENSSL_cleanse(s->tls13_gm_master_secret_ikm, sizeof(s->tls13_gm_master_secret_ikm));
    s->tls13_gm_master_secret_ikm_len = 0;
#else
    /* Calls SSLfatal() if required */
    if (!tls13_generate_secret(s, md, prev, NULL, 0, out))
        return 0;
#endif
    return 1;
}
...
#ifndef OPENSSL_NO_SM2
        /*
         * GM dual-cert server defers master_secret until Client Finished; at
         * that point ssl_handshake_hash() includes Client Finished. Server
         * application traffic must use Transcript-Hash through Server Finished
         * only (saved in SW_FINISHED post_work).
         */
        if (label == server_application_traffic && s->server
                && !(which & SSL3_CC_READ) && s->ext.tls13_gm_dual_cert
                && s->tls13_gm_dual_cert_sf_hash_saved) {
            memcpy(hashval, s->server_finished_hash, hashlen);
            gm_sf_hash_fixup = 1;
        }
#endif
    }
...
#ifndef OPENSSL_NO_SM2
    if (gm_sf_hash_fixup)
        s->tls13_gm_dual_cert_sf_hash_saved = 0;
#endif
