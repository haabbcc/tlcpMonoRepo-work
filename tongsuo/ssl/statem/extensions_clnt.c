...
static int sm2kyber_debug_enabled(void)
{
    ...
    if (init)
        return enabled;
    init = 1;
    env = getenv("TS_HYBRID_KEX_DEBUG");
    enabled = (env != NULL && env[0] != '\0' && strcmp(env, "0") != 0);
    return enabled;
}

static void sm2kyber_debug_dump(const char *tag, const unsigned char *buf, size_t len)
{
    size_t i, show;

    if (!sm2kyber_debug_enabled())
        return;
    show = len < 24 ? len : 24;
    fprintf(stderr, "[SM2KYBER-DBG][clnt] %s len=%zu hex=", tag, len);
    for (i = 0; i < show; i++)
...
# ifndef OPENSSL_NO_SM2
        /*
         * Kyber may be absent from the wire if tls_valid_group or
         * tls_group_allowed rejects KEM entries; key_share still sends it,
         * which makes the server reject the share. Force inclusion when hybrid
         * mode is enabled.
         */
        const int kyber_tls13_hybrid = (s->enable_sm2_kyber768_tls13
                                        && max_version >= TLS1_3_VERSION
                                        && ctmp == OSSL_TLS_GROUP_ID_kyber768);
# else
        const int kyber_tls13_hybrid = 0;
# endif
        ...
        if ((tls_valid_group(s, ctmp, min_version, max_version, 0, &okfortls13)
            ...
            if ((okfortls13 || kyber_tls13_hybrid) && max_version == TLS1_3_VERSION)
                tls13added++;
            added++;
        }
    }
...
# ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("ClientHello key_share (classical pubkey)",
                        encoded_point, encodedlen);
# endif
...
# ifndef OPENSSL_NO_SM2
static int add_key_share_kyber768(SSL *s, WPACKET *pkt)
{
    unsigned char pk[pqcrystals_kyber768_PUBLICKEYBYTES];
    unsigned char sk[pqcrystals_kyber768_SECRETKEYBYTES];

    if (s->s3.kyber768_cli_sk != NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    if (!WPACKET_put_bytes_u16(pkt, OSSL_TLS_GROUP_ID_kyber768)
            || !WPACKET_sub_memcpy_u16(pkt, pk, sizeof(pk))) {
        OPENSSL_cleanse(sk, sizeof(sk));
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
# ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("ClientHello key_share (kyber768 pk)", pk, sizeof(pk));
    sm2kyber_debug_dump("Client local kyber768 sk", sk, sizeof(sk));
# endif
    s->s3.kyber768_cli_sk = OPENSSL_memdup(sk, sizeof(sk));
    OPENSSL_cleanse(sk, sizeof(sk));
    if (s->s3.kyber768_cli_sk == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    s->s3.kyber768_cli_sk_len = pqcrystals_kyber768_SECRETKEYBYTES;
    return 1;
}
# endif
#endif
...
# ifndef OPENSSL_NO_SM2
    if (s->enable_sm2_kyber768_tls13) {
        if (sm2kyber_debug_enabled())
            fprintf(stderr, "[SM2KYBER-DBG][clnt] constructing dual key_share entries: SM2 + KYBER768\n");
        int have_sm2 = 0, have_kyber = 0;

        if (s->hello_retry_request == SSL_HRR_PENDING) {
            SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_NO_SUITABLE_KEY_SHARE);
            return EXT_RETURN_FAIL;
        }
        tls1_get_supported_groups(s, &pgroups, &num_groups);
        for (i = 0; i < num_groups; i++) {
            if (pgroups[i] == TLSEXT_curve_SM2)
                have_sm2 = 1;
            if (pgroups[i] == OSSL_TLS_GROUP_ID_kyber768)
                have_kyber = 1;
        }
        if (!have_sm2 || !have_kyber) {
        ...
        if (!add_key_share(s, pkt, TLSEXT_curve_SM2)
                || !add_key_share_kyber768(s, pkt)) {
            return EXT_RETURN_FAIL;
        }
        if (!WPACKET_close(pkt) || !WPACKET_close(pkt)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
...
#ifndef OPENSSL_NO_SM2
        sm2kyber_debug_dump("ServerHello key_share pubkey",
                            PACKET_data(&encoded_pt), PACKET_remaining(&encoded_pt));
#endif

        if (ssl_derive(s, ckey, skey,
#ifndef OPENSSL_NO_SM2
                       s->enable_sm2_kyber768_tls13 ? 0 :
#endif
                       1) == 0) {
            /* SSLfatal() already called */
            EVP_PKEY_free(skey);
...
int tls_parse_stoc_kyber768_ciphertext(SSL *s, PACKET *pkt,
                                         unsigned int context, X509 *x,
                                         size_t chainidx)
{
    unsigned char ss[pqcrystals_kyber768_BYTES];

    if (!s->enable_sm2_kyber768_tls13)
        return 1;
    if (PACKET_remaining(pkt) != pqcrystals_kyber768_CIPHERTEXTBYTES) {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
        return 0;
    }
#ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("ServerHello ext 0xff05 kyber ciphertext",
                        PACKET_data(pkt), PACKET_remaining(pkt));
#endif
    if (s->s3.kyber768_cli_sk == NULL || s->s3.tmp.pms == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    if (pqcrystals_kyber768_ref_dec(ss, PACKET_data(pkt),
                                    s->s3.kyber768_cli_sk) != 0) {
        SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
        return 0;
    }
#ifndef OPENSSL_NO_SM2
    sm2kyber_debug_dump("Client decapsulated kyber shared secret",
                        ss, sizeof(ss));
#endif
    if (!ssl_sm2_kyber768_tls13_combine_and_gensecret(s, ss, sizeof(ss))) {
