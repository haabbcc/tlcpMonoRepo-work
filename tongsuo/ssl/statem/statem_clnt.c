...
MSG_PROCESS_RETURN tls_process_server_hello(SSL *s, PACKET *pkt)
{
    ...
    if (SSL_IS_TLS13(s) && s->enable_sm_tls13_strict == 1) {
        ...
        if (cipher->id == TLS1_3_CK_SM4_GCM_SM3
            ...
            /*
             * Hybrid SM2+Kyber768 still negotiates curveSM2 as the ECDH group_id;
             * Kyber is a second KeyShareEntry / EE extension, not this field.
             */
...
WORK_STATE tls_post_process_server_certificate(SSL *s, WORK_STATE wst)
{
...
#ifndef OPENSSL_NO_SM2
    if (SSL_IS_TLS13(s) && s->ext.tls13_gm_dual_cert) {
        if (sk_X509_num(s->session->peer_chain) < 2) {
            SSLfatal(s, SSL_AD_BAD_CERTIFICATE, SSL_R_BAD_DATA);
            return WORK_ERROR;
        }
        EVP_PKEY_free(s->tls13_peer_gm_enc_pkey);
        s->tls13_peer_gm_enc_pkey =
            X509_get_pubkey(sk_X509_value(s->session->peer_chain, 1));
        if (s->tls13_peer_gm_enc_pkey == NULL) {
            SSLfatal(s, SSL_AD_BAD_CERTIFICATE, SSL_R_WRONG_CERTIFICATE_TYPE);
            return WORK_ERROR;
        }
    }
#endif
    ...
    if (SSL_IS_TLS13(s) && s->enable_sm_tls13_strict == 1) {
        ...
        if (cipher->id == TLS1_3_CK_SM4_GCM_SM3
            ...
            if (EVP_PKEY_id(pkey) != EVP_PKEY_SM2 && !pkey_is_pqc) {
                SSLfatal(s, SSL_AD_BAD_CERTIFICATE,
                         SSL_R_WRONG_CERTIFICATE_TYPE);
                return WORK_ERROR;
            }

            if (sig_nid != NID_SM2_with_SM3
                && sig_nid != NID_ML_DSA_44_with_SM3) {
                SSLfatal(s, SSL_AD_BAD_CERTIFICATE,
        ...
        if (s->enable_sm_tls13_strict == 1) {
            ...
            if (cipher->id == TLS1_3_CK_SM4_GCM_SM3
                ...
                if (s->s3.tmp.peer_sigalgslen > 0) {
                    int found_gm_sig = 0;
                    size_t si;
                    for (si = 0; si < s->s3.tmp.peer_sigalgslen; si++) {
                        if (s->s3.tmp.peer_sigalgs[si] == TLSEXT_SIGALG_sm2sig_sm3
                            || s->s3.tmp.peer_sigalgs[si] == TLSEXT_SIGALG_mldsa44) {
                            found_gm_sig = 1;
                            break;
                        }
                    }
                ...
                if (s->s3.tmp.peer_cert_sigalgslen > 0) {
                    int found_gm_certsig = 0;
                    size_t ci;
                    for (ci = 0; ci < s->s3.tmp.peer_cert_sigalgslen; ci++) {
                        if (s->s3.tmp.peer_cert_sigalgs[ci] == TLSEXT_SIGALG_sm2sig_sm3
                            || s->s3.tmp.peer_cert_sigalgs[ci] == TLSEXT_SIGALG_mldsa44) {
                            found_gm_certsig = 1;
                            break;
                        }
                    }
