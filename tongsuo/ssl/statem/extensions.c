...
static int init_tls1_3_gm_dual_cert(SSL *s, unsigned int context);
static EXT_RETURN tls_construct_ctos_tls1_3_gm_dual_cert(SSL *s, WPACKET *pkt,
                                                         unsigned int context,
                                                         X509 *x, size_t chainidx);
static EXT_RETURN tls_construct_stoc_tls1_3_gm_dual_cert(SSL *s, WPACKET *pkt,
                                                          unsigned int context,
                                                          X509 *x, size_t chainidx);
static int tls_parse_ctos_tls1_3_gm_dual_cert(SSL *s, PACKET *pkt,
                                              unsigned int context, X509 *x,
                                              size_t chainidx);
static int tls_parse_stoc_tls1_3_gm_dual_cert(SSL *s, PACKET *pkt,
                                              unsigned int context, X509 *x,
                                              size_t chainidx);
static int final_tls1_3_gm_dual_cert(SSL *s, unsigned int context, int sent);
static int final_tls1_3_kyber768_ciphertext(SSL *s, unsigned int context,
                                            int sent);
#endif
...
#ifndef OPENSSL_NO_SM2
    /*
     * SM2+Kyber768 hybrid: IKM = ECDH_SS || Kyber_SS for tls13_generate_handshake_secret.
     * The client installs handshake read keys only after tls_process_server_hello() finishes
     * parsing all ServerHello extensions (see change_cipher_state there). So the Kyber
     * ciphertext must be available in the same pass as key_share—i.e. a ServerHello
     * extension—not inside EncryptedExtensions (which is encrypted with keys that require
     * the full IKM first).
     */
    {
        TLSEXT_TYPE_tls1_3_kyber768_ciphertext,
        SSL_EXT_TLS1_3_SERVER_HELLO | SSL_EXT_TLS1_3_ONLY,
        NULL,
        NULL, tls_parse_stoc_kyber768_ciphertext,
        tls_construct_stoc_kyber768_ciphertext, NULL,
        final_tls1_3_kyber768_ciphertext
    },
#else
    INVALID_EXTENSION,
#endif
...
#ifndef OPENSSL_NO_SM2
    {
        TLSEXT_TYPE_tls1_3_gm_dual_cert,
        SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS
        | SSL_EXT_TLS1_3_ONLY,
        init_tls1_3_gm_dual_cert,
        tls_parse_ctos_tls1_3_gm_dual_cert, tls_parse_stoc_tls1_3_gm_dual_cert,
        tls_construct_stoc_tls1_3_gm_dual_cert, tls_construct_ctos_tls1_3_gm_dual_cert,
        final_tls1_3_gm_dual_cert
    },
#else
    INVALID_EXTENSION,
#endif
...
#ifndef OPENSSL_NO_SM2
                /* Server-only SM2+Kyber768 hybrid; client never sends this. */
                && type != TLSEXT_TYPE_tls1_3_kyber768_ciphertext
#endif
...
# ifndef OPENSSL_NO_SM2
    if (s->server && SSL_IS_TLS13(s) && s->ctx->enable_sm2_kyber768_tls13
            && s->s3.peer_tmp != NULL) {
        if (s->s3.peer_kyber768_pk == NULL) {
...
{
    if ((context & SSL_EXT_CLIENT_HELLO) != 0) {
        s->ext.tls13_gm_dual_cert = 0;
        s->ext.tls13_gm_dual_cert_request = 0;
    }
    return 1;
}
                                              ...
                                              size_t chainidx)
{
    ...
    s->ext.tls13_gm_dual_cert_request = 1;
    return 1;
}
                                                         ...
                                                         X509 *x, size_t chainidx)
{
    ...
    /*
     * TLS_client_method() uses TLS_ANY_VERSION, so SSL_IS_TLS13(s) is false
     * while building ClientHello. Match tls_construct_ctos_supported_versions.
     */
    if (!s->ctx->enable_gm_tls13_dual_cert)
        return EXT_RETURN_NOT_SENT;
    reason = ssl_get_min_max_version(s, &min_version, &max_version, NULL);
    if (reason != 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, reason);
    ...
    if (max_version < TLS1_3_VERSION)
        return EXT_RETURN_NOT_SENT;
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_tls1_3_gm_dual_cert)
            || !WPACKET_start_sub_packet_u16(pkt)
            || !WPACKET_close(pkt)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
                                                         ...
                                                         X509 *x, size_t chainidx)
{
    ...
    /* TLS_server_method() is TLS_ANY_VERSION; SSL_IS_TLS13(s) is false here. */
    if (!s->server)
        return EXT_RETURN_NOT_SENT;
    /*
     * Echo when the client requested dual-cert and we have sign+enc SM2 certs
     * and a GM TLS 1.3 suite — same conditions as final_tls1_3_gm_dual_cert
     * (EncryptedExtensions may be built before that final runs).
     */
    if (!s->ext.tls13_gm_dual_cert_request || !s->ctx->enable_gm_tls13_dual_cert
            || !ssl_has_cert(s, SSL_PKEY_SM2_SIGN)
            || !ssl_has_cert(s, SSL_PKEY_SM2_ENC))
        return EXT_RETURN_NOT_SENT;
    c = s->s3.tmp.new_cipher;
    if (c == NULL || (c->id != TLS1_3_CK_SM4_GCM_SM3
...
static int final_tls1_3_kyber768_ciphertext(SSL *s, unsigned int context,
                                            int sent)
{
    if (!SSL_IS_TLS13(s) || !s->enable_sm2_kyber768_tls13)
        return 1;
    if ((context & SSL_EXT_TLS1_3_SERVER_HELLO) == 0)
        return 1;
    if (s->server)
        return 1;
    if (!sent) {
        SSLfatal(s, TLS13_AD_MISSING_EXTENSION, SSL_R_EXTENSION_NOT_RECEIVED);
        return 0;
    }
    return 1;
}
...
static int final_tls1_3_gm_dual_cert(SSL *s, unsigned int context, int sent)
{
    if ((context & SSL_EXT_CLIENT_HELLO) == 0)
        return 1;
    /* TLS_server_method(): SSL_IS_TLS13(s) is false; rely on cipher below. */
    if (!s->server)
        return 1;
    if (s->ext.tls13_gm_dual_cert_request && s->ctx->enable_gm_tls13_dual_cert
            && ssl_has_cert(s, SSL_PKEY_SM2_SIGN) && ssl_has_cert(s, SSL_PKEY_SM2_ENC)) {
