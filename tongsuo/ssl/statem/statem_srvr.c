    ...
    case TLS_ST_SW_FINISHED:
        ...
        if (SSL_IS_TLS13(s)) {
            ...
            if (s->ext.tls13_gm_dual_cert) {
                ...
                /*
                 * Deferred master_secret: save Transcript-Hash through Server
                 * Finished now; later ssl_handshake_hash() would include Client
                 * Finished and must not be used for "s ap traffic".
                 */
                if (!ssl3_digest_cached_records(s, 1)
                        || !ssl_handshake_hash(s, s->server_finished_hash,
                                EVP_MAX_MD_SIZE, &sf_hlen)) {
                    /* SSLfatal() already called */
                    return WORK_ERROR;
                }
                s->tls13_gm_dual_cert_sf_hash_saved = 1;
            } else
#endif
            {
...
#ifndef OPENSSL_NO_SM2
        /*
         * GM TLS 1.3 dual-cert Client Finished: verify_data || u16 || SM2
         * ciphertext; default FINISHED_MAX_LENGTH is far too small.
         */
...
static int tls_early_post_process_client_hello(SSL *s)
{
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (s->enable_sm_tls13_strict == 1
            ...
            /*
             * RFC 8998: classical KEX group is curveSM2. When SM2+Kyber768
             * hybrid is enabled, tls_parse_ctos_key_share still needs Kyber768
             * in our supported_groups list to accept the second KeyShareEntry.
             */
            if (s->enable_sm2_kyber768_tls13) {
                if (!tls1_set_groups_list(s->ctx, &s->ext.supportedgroups,
...
#ifndef OPENSSL_NO_SM2
    if (SSL_IS_TLS13(s) && s->ext.tls13_gm_dual_cert) {
        if (s->s3.tmp.tls13_gm_enc_cert == NULL
                || s->s3.tmp.tls13_gm_enc_cert->x509 == NULL) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
        if (!ssl3_output_cert_chain_gm_dual(s, pkt, cpk,
                                           s->s3.tmp.tls13_gm_enc_cert->x509)) {
            /* SSLfatal() already called */
            return 0;
        }
        return 1;
