...
static void sm2kyber_debug_dump(const char *tag, const unsigned char *buf, size_t len)
{
    ...
    if (!sm2kyber_debug_enabled())
        return;
    show = len < 24 ? len : 24;
    fprintf(stderr, "[SM2KYBER-DBG][srvr] %s len=%zu hex=", tag, len);
    for (i = 0; i < show; i++)
...
# ifndef OPENSSL_NO_SM2
        if (s->ctx->enable_sm2_kyber768_tls13
                && group_id == OSSL_TLS_GROUP_ID_kyber768) {
            if (s->s3.peer_kyber768_pk != NULL) {
                SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
                return 0;
            }
            if (!check_in_list(s, group_id, srvrgroups, srvr_num_groups, 0)
                    || !check_in_list(s, group_id, clntgroups, clnt_num_groups,
                                      0)) {
                SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
                return 0;
            }
            if (PACKET_remaining(&encoded_pt) != pqcrystals_kyber768_PUBLICKEYBYTES) {
                SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
                return 0;
            }
            s->s3.peer_kyber768_pk = OPENSSL_memdup(PACKET_data(&encoded_pt),
                                                    PACKET_remaining(&encoded_pt));
            if (s->s3.peer_kyber768_pk == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
                return 0;
            }
            s->s3.peer_kyber768_pk_len = PACKET_remaining(&encoded_pt);
# ifndef OPENSSL_NO_SM2
            sm2kyber_debug_dump("ClientHello key_share kyber768 pk",
...
# ifndef OPENSSL_NO_SM2
        if (s->ctx->enable_sm2_kyber768_tls13
                && group_id != (unsigned int)TLSEXT_curve_SM2) {
            /* Classical leg must be SM2 when hybrid is enabled */
            continue;
        }
# endif
...
# ifndef OPENSSL_NO_SM2
    if (s->ctx->enable_sm2_kyber768_tls13) {
        if (!found || !found_kyber || s->s3.group_id != TLSEXT_curve_SM2) {
            SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
            return 0;
        }
    }
                                        ...
                                        size_t chainidx)
{
    ...
    if (!ginf->is_kem) {
...
#ifndef OPENSSL_NO_SM2
        sm2kyber_debug_dump("ServerHello key_share classical pubkey",
                            encodedPoint, encoded_pt_len);
#endif
        OPENSSL_free(encodedPoint);
         ...
         */
        s->s3.tmp.pkey = skey;
        if (ssl_derive(s, skey, ckey,
#ifndef OPENSSL_NO_SM2
                       (s->enable_sm2_kyber768_tls13
                        && s->s3.peer_kyber768_pk != NULL) ? 0 :
#endif
                       1) == 0) {
            /* SSLfatal() already called */
            return EXT_RETURN_FAIL;
...
#ifndef OPENSSL_NO_SM2
        if (s->enable_sm2_kyber768_tls13 && s->s3.peer_kyber768_pk != NULL
                && sm2kyber_debug_enabled()) {
            fprintf(stderr, "[SM2KYBER-DBG][srvr] ECDH leg cached for hybrid: pmslen=%zu\n",
                    s->s3.tmp.pmslen);
        }
#endif
    } else {
...
/*
 * Kyber768 ciphertext is a ServerHello extension (not EncryptedExtensions):
 * both ends need ECDH_SS || Kyber_SS before tls13_change_cipher_state for the
 * handshake can run; EE is encrypted with those keys.
 */
EXT_RETURN tls_construct_stoc_kyber768_ciphertext(SSL *s, WPACKET *pkt,
                                                  unsigned int context, X509 *x,
                                                  size_t chainidx)
{
    ...
    if (!s->enable_sm2_kyber768_tls13 || !s->server)
        return EXT_RETURN_NOT_SENT;
    if (s->hello_retry_request == SSL_HRR_PENDING)
        return EXT_RETURN_NOT_SENT;
    if (s->s3.peer_kyber768_pk == NULL || s->s3.tmp.pms == NULL)
        return EXT_RETURN_NOT_SENT;

    ct = OPENSSL_malloc(pqcrystals_kyber768_CIPHERTEXTBYTES);
    if (ct == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
        return EXT_RETURN_FAIL;
    }
    if (pqcrystals_kyber768_ref_enc(ct, ss, s->s3.peer_kyber768_pk) != 0) {
        OPENSSL_free(ct);
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
    ...
    sm2kyber_debug_dump("ServerHello ext 0xff05 kyber ciphertext",
                        ct, pqcrystals_kyber768_CIPHERTEXTBYTES);
    sm2kyber_debug_dump("Server encapsulated kyber shared secret", ss, sizeof(ss));
#endif
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_tls1_3_kyber768_ciphertext)
            || !WPACKET_start_sub_packet_u16(pkt)
            || !WPACKET_memcpy(pkt, ct, pqcrystals_kyber768_CIPHERTEXTBYTES)
            || !WPACKET_close(pkt)) {
        OPENSSL_free(ct);
