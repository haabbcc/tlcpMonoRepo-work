...
int ossl_statem_accept(SSL *s)
{
    ...
    fprintf(stderr, "[NTLS Server] ossl_statem_accept: enable_ntls=%d, enable_force_ntls=%d\n",
           s->enable_ntls, s->enable_force_ntls);
    
    if (s->enable_force_ntls == 1) {
        fprintf(stderr, "[NTLS Server] Using force NTLS mode\n");
        return state_machine_ntls(s, 1);
    } else if (s->enable_ntls == 1) {
        ret = SSL_connection_is_ntls(s, 1);
        fprintf(stderr, "[NTLS Server] SSL_connection_is_ntls returned: %d\n", ret);
        if (ret == 0) {
            fprintf(stderr, "[NTLS Server] Not NTLS, using standard TLS\n");
            return state_machine(s, 1);
        } else if (ret == 1) {
            fprintf(stderr, "[NTLS Server] Is NTLS, using NTLS state machine\n");
            return state_machine_ntls(s, 1);
        } else {
            fprintf(stderr, "[NTLS Server] SSL_connection_is_ntls error: %d\n", ret);
            return ret;
        }
    } else
#endif
    {
        fprintf(stderr, "[NTLS Server] NTLS not enabled, using standard TLS\n");
    return state_machine(s, 1);
    }
}
