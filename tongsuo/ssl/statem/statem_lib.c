...
int tls_construct_cert_verify(SSL *s, WPACKET *pkt)
{
    ...
    if (s->version == SSL3_VERSION) {
         ...
         * pqmagic ML-DSA may not support a NULL-buffer length query; use a
         * single-shot sign with a pre-sized buffer (see NTLS path).
         */
        if (lu->sigalg == TLSEXT_SIGALG_mldsa44) {
            ...
            if (sl > 0)
                ...
                siglen = 2420;
        } else if (EVP_DigestSign(mctx, NULL, &siglen, hdata, hdatalen) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
            goto err;
        }
        sig = OPENSSL_malloc(siglen);
...
#ifndef OPENSSL_NO_SM2
    if (SSL_IS_TLS13(s) && !s->server && s->ext.tls13_gm_dual_cert) {
        EVP_PKEY *pub = s->tls13_peer_gm_enc_pkey;
        EVP_PKEY_CTX *pctx = NULL;
        unsigned char *ct = NULL;
        size_t ctlen;
        size_t hl = finish_md_len;
        size_t dummy;
        int use_kem = 0;
        ...
        use_kem = !EVP_PKEY_is_sm2(pub);

        pctx = EVP_PKEY_CTX_new_from_pkey(s->ctx->libctx, pub, s->ctx->propq);
        if (pctx == NULL) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
        ...
        if (use_kem) {
            size_t sslen = sizeof(s->tls13_gm_master_secret_ikm);

            if (EVP_PKEY_encapsulate_init(pctx, NULL) <= 0
                    || EVP_PKEY_encapsulate(pctx, NULL, &ctlen,
                                            NULL, &sslen) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                EVP_PKEY_CTX_free(pctx);
                return 0;
            ...
            if (ct == NULL
                    || EVP_PKEY_encapsulate(pctx, ct, &ctlen,
                            s->tls13_gm_master_secret_ikm, &sslen) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                OPENSSL_free(ct);
                EVP_PKEY_CTX_free(pctx);
                return 0;
            }
            s->tls13_gm_master_secret_ikm_len = sslen;
        } else {
            if (RAND_bytes_ex(s->ctx->libctx, s->tls13_gm_master_secret_ikm,
                              hl, 0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                EVP_PKEY_CTX_free(pctx);
                return 0;
            }
            s->tls13_gm_master_secret_ikm_len = hl;
            if (EVP_PKEY_encrypt_init(pctx) <= 0
                    || EVP_PKEY_encrypt(pctx, NULL, &ctlen,
                            s->tls13_gm_master_secret_ikm, hl) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                EVP_PKEY_CTX_free(pctx);
                return 0;
            }
            ct = OPENSSL_malloc(ctlen);
            if (ct == NULL
                    || EVP_PKEY_encrypt(pctx, ct, &ctlen,
                            s->tls13_gm_master_secret_ikm, hl) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                OPENSSL_free(ct);
                EVP_PKEY_CTX_free(pctx);
...
MSG_PROCESS_RETURN tls_process_finished(SSL *s, PACKET *pkt)
{
...
#ifndef OPENSSL_NO_SM2
    if (SSL_IS_TLS13(s) && s->server && s->ext.tls13_gm_dual_cert) {
        if (PACKET_remaining(pkt) <= md_len) {
    ...
    if (SSL_IS_TLS13(s) && s->server && s->ext.tls13_gm_dual_cert) {
        PACKET enc;
        unsigned int enc_be16;
        const unsigned char *ct;
        size_t ctlen;
        EVP_PKEY *priv = (s->s3.tmp.tls13_gm_enc_cert != NULL)
            ? s->s3.tmp.tls13_gm_enc_cert->privatekey : NULL;
        EVP_PKEY_CTX *pctx = NULL;
        size_t ptl = sizeof(s->tls13_gm_master_secret_ikm);
        size_t dummy;
        int use_kem = 0;
        ...
        if (PACKET_remaining(pkt) < md_len + 2) {
            SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_DIGEST_LENGTH);
            return MSG_PROCESS_ERROR;
        }
        if (!PACKET_buf_init(&enc, PACKET_data(pkt) + md_len,
                             PACKET_remaining(pkt) - md_len)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return MSG_PROCESS_ERROR;
        }
        if (!PACKET_get_net_2(&enc, &enc_be16)) {
            SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_DIGEST_LENGTH);
            return MSG_PROCESS_ERROR;
        }
        ctlen = (size_t)enc_be16;
        if (ctlen == 0 || PACKET_remaining(&enc) != ctlen) {
            SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_DIGEST_LENGTH);
            return MSG_PROCESS_ERROR;
        }
        ct = PACKET_data(&enc);

        use_kem = !EVP_PKEY_is_sm2(priv);

        pctx = EVP_PKEY_CTX_new_from_pkey(s->ctx->libctx, priv, s->ctx->propq);
        if (pctx == NULL) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return MSG_PROCESS_ERROR;
        }
        ...
        if (use_kem) {
            ptl = sizeof(s->tls13_gm_master_secret_ikm);
            if (EVP_PKEY_decapsulate_init(pctx, NULL) <= 0
                    || EVP_PKEY_decapsulate(pctx,
                            s->tls13_gm_master_secret_ikm, &ptl,
                            ct, ctlen) <= 0) {
            ...
            ptl = 0;
            if (EVP_PKEY_decrypt(pctx, NULL, &ptl, ct, ctlen) <= 0) {
                SSLfatal(s, SSL_AD_DECRYPT_ERROR,
                         SSL_R_SSLV3_ALERT_BAD_RECORD_MAC);
                EVP_PKEY_CTX_free(pctx);
                return MSG_PROCESS_ERROR;
            }
            if (EVP_PKEY_decrypt(pctx, s->tls13_gm_master_secret_ikm, &ptl,
                                 ct, ctlen) <= 0) {
                SSLfatal(s, SSL_AD_DECRYPT_ERROR,
                         SSL_R_SSLV3_ALERT_BAD_RECORD_MAC);
                EVP_PKEY_CTX_free(pctx);
        ...
        EVP_PKEY_CTX_free(pctx);
        s->tls13_gm_master_secret_ikm_len = ptl;
        if (!s->method->ssl3_enc->generate_master_secret(s,
                s->master_secret, s->handshake_secret, 0,
                &dummy)) {
            OPENSSL_cleanse(s->tls13_gm_master_secret_ikm,
                            sizeof(s->tls13_gm_master_secret_ikm));
            s->tls13_gm_master_secret_ikm_len = 0;
            return MSG_PROCESS_ERROR;
        }
    }
#endif
...
#ifndef OPENSSL_NO_SM2
int ssl3_output_cert_chain_gm_dual(SSL *s, WPACKET *pkt, CERT_PKEY *cpk,
                                   X509 *enc)
{
    if (enc == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    ...
    if (!ssl_add_cert_chain(s, pkt, cpk))
        return 0;
    if (!ssl_add_cert_to_wpacket(s, pkt, enc, 0)) {
        /* SSLfatal() already called */
        return 0;
    }
    if (!WPACKET_close(pkt)) {
