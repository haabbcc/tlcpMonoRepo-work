...
typedef enum OPTION_choice {
    ...
    OPT_ENABLE_GM_TLS13_DUAL_CERT,
    OPT_ENABLE_SM2_KYBER768_TLS13,
#endif
...
const OPTIONS s_server_options[] = {
    ...
    {"enable_gm_tls13_dual_cert", OPT_ENABLE_GM_TLS13_DUAL_CERT, '-',
     "Enable TLS 1.3 GM dual certificate (sign+enc) mode"},
    {"enable_sm2_kyber768_tls13", OPT_ENABLE_SM2_KYBER768_TLS13, '-',
     "Enable TLS 1.3 SM2 + Kyber768 hybrid key exchange (Tongsuo)"},
#endif
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
...
int s_server_main(int argc, char *argv[])
{
    ...
    int enable_gm_tls13_dual_cert = 0;
    int enable_sm2_kyber768_tls13 = 0;
#endif
#ifndef OPENSSL_NO_OCSP
 ...
 opthelp:
        ...
        case OPT_ENABLE_SM_TLS13_STRICT:
            ...
            break;
        case OPT_ENABLE_GM_TLS13_DUAL_CERT:
            enable_gm_tls13_dual_cert = 1;
            break;
        case OPT_ENABLE_SM2_KYBER768_TLS13:
            enable_sm2_kyber768_tls13 = 1;
            break;
#endif
#ifndef OPENSSL_NO_DELEGATED_CREDENTIAL
...
skip:
    ...
    if (enable_gm_tls13_dual_cert)
        SSL_CTX_set_gm_tls13_dual_cert(ctx, 1);
    if (enable_sm2_kyber768_tls13)
        SSL_CTX_enable_sm2_kyber768_tls13(ctx);
#endif
...
static int sv_body(int s, int stype, int prot, unsigned char *context)
{
    ...
    fprintf(stderr, "[s_server] sv_body called, socket=%d\n", s);
...
# endif
    ...
    for (;;) {
        ...
        fprintf(stderr, "[s_server] Loop: read_from_sslcon=%d, has_pending=%d, async=%d, waiting_for_async=%d\n",
               read_from_sslcon, SSL_has_pending(con), async, SSL_waiting_for_async(con));

        if (!read_from_sslcon) {
            fprintf(stderr, "[s_server] Waiting for data on socket using select\n");
            FD_ZERO(&readfds);
#if !defined(OPENSSL_SYS_WINDOWS) && !defined(OPENSSL_SYS_MSDOS)
            ...
            fprintf(stderr, "[s_server] select returned: %d\n", i);

            if ((SSL_is_dtls(con)) && DTLSv1_handle_timeout(con) > 0)
                BIO_printf(bio_err, "TIMEOUT occurred\n");

            if (i <= 0) {
                fprintf(stderr, "[s_server] select returned <= 0, continuing\n");
                continue;
            }
            if (FD_ISSET(fileno_stdin(), &readfds)) {
                read_from_terminal = 1;
                fprintf(stderr, "[s_server] stdin is readable\n");
            }
#endif
            if (FD_ISSET(s, &readfds)) {
                read_from_sslcon = 1;
                fprintf(stderr, "[s_server] Socket is readable, setting read_from_sslcon=1\n");
        }
        }
        fprintf(stderr, "[s_server] After select: read_from_terminal=%d, read_from_sslcon=%d\n",
               read_from_terminal, read_from_sslcon);
        if (read_from_terminal) {
            fprintf(stderr, "[s_server] Processing terminal input\n");
            if (s_crlf) {
                int j, lf_num;
...
#endif
            fprintf(stderr, "[s_server] About to write %d bytes to SSL connection\n", i);
            l = k = 0;
            for (;;) {
...
#endif
                fprintf(stderr, "[s_server] Calling SSL_write: remaining=%d, offset=%d\n", i, l);
                k = SSL_write(con, &(buf[l]), (unsigned int)i);
                fprintf(stderr, "[s_server] SSL_write returned: %d\n", k);
#ifndef OPENSSL_NO_SRP
                ...
                case SSL_ERROR_NONE:
                    fprintf(stderr, "[s_server] SSL_write succeeded, wrote %d bytes\n", k);
                    break;
                case SSL_ERROR_WANT_ASYNC:
        ...
        fprintf(stderr, "[s_server] After terminal check: read_from_sslcon=%d\n", read_from_sslcon);
        if (read_from_sslcon) {
            /*
             ...
             * init_ssl_connection
             */
            fprintf(stderr, "[s_server] read_from_sslcon=true, async=%d, waiting_for_async=%d, init_finished=%d\n",
                   async, SSL_waiting_for_async(con), SSL_is_init_finished(con));
            if ((!async || !SSL_waiting_for_async(con))
                ...
                fprintf(stderr, "[s_server] About to call init_ssl_connection\n");
                i = init_ssl_connection(con);
                fprintf(stderr, "[s_server] init_ssl_connection returned: %d\n", i);
...
static int init_ssl_connection(SSL *con)
{
    ...
    fprintf(stderr, "[s_server] init_ssl_connection called\n");
    ...
    if (dtlslisten || stateless) {
        ...
        if (i > 0) {
            ...
            fprintf(stderr, "[s_server] Calling SSL_accept (stateless/dtls path)\n");
            i = SSL_accept(con);
            fprintf(stderr, "[s_server] SSL_accept returned: %d\n", i);
        } else {
            BIO_ADDR_free(client);
        }
    } else {
        do {
            fprintf(stderr, "[s_server] Calling SSL_accept (normal path)\n");
            i = SSL_accept(con);
            fprintf(stderr, "[s_server] SSL_accept returned: %d\n", i);
